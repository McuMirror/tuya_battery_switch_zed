# Set Project Name
PROJECT_NAME = tuya_battery_switch_zed
			...
			ifeq ($(PROJECT_MODEL),_model_4)
				...
				ifeq ($(PROJECT_MODEL),_model_5)
					...
					ifeq ($(PROJECT_MODEL),_model_6)
						MANUF_CODE ?= 4417
						IMAGE_TYPE ?= 54179
						PROJECT_DEF = "-DDEVICE_MODEL=DEVICE_MODEL_6"
						MODEL = _M006
					else
						ifeq ($(PROJECT_MODEL),_model_7)
							MANUF_CODE ?= 4417
							IMAGE_TYPE ?= 54179
...
flash-orig-write:
	@python3 $(TOOLS_PATH)/TlsrPgm.py -p$(DOWNLOAD_PORT) -z11 -a 100 -s -m we 0 $(BIN_PATH)/tuya_battery_switch_orig.bin
	
flash-orig-read-512:
	@python3 $(TOOLS_PATH)/TlsrPgm.py -p$(DOWNLOAD_PORT) -z11 -a 100 -s -m rf 0 0x80000 tuya_battery_switch_orig.bin
	
flash-orig-read-1024:
	@python3 $(TOOLS_PATH)/TlsrPgm.py -p$(DOWNLOAD_PORT) -z11 -a 100 -s -m rf 0 0x100000 tuya_battery_switch_orig.bin
